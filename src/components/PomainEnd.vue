        ...
        <el-table :data="pomainList" stripe style="width: 100%" size="mini" border loading>
          ...
          <el-table-column label="操作" width="150">
            <template slot-scope="scope">
              <el-button size="mini" @click="handleEnd(scope.$index, scope.row)">采购单了结</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
    ...
    handleEnd(index, row) {
      this.$confirm("确认了结此采购单么?", "提示", {
        ...
        axios({
          ...
          method: "post",
          data: qs.stringify({ payType: row.payType, poId: row.poId })
        })
          .then(resp => {
            console.log(resp);
