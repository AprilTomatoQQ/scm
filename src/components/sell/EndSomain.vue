        ...
        <el-table :data="somainList" stripe style="width: 100%" size="mini" border loading>
          ...
          <el-table-column label="操作" width="150">
            <template slot-scope="scope">
              <el-button size="mini" @click="handleEnd(scope.$index, scope.row)">销售单了结</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
  ...
  methods: {
    ...
    getList() {
      axios({
        ...
        method: "get",
        params: { type:'4', payType:this.payType }
      }).then(resp => {
        console.log(resp);
    ...
    handleEnd(index, row) {
      this.$confirm("确认了结此销售单么?", "提示", {
        ...
        axios({
          ...
          method: "post",
          data: qs.stringify({ payType: row.payType, soId: row.soId })
        })
          .then(resp => {
            console.log(resp);
