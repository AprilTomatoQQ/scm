<template>
  <div>
    <el-form :inline="true" class="demo-form-inline">
      <el-form-item>
        <el-button type="primary" plain size="mini" @click="addSomain">添加</el-button>
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    addSomain() {
      location.href = "http://localhost:8080/#/sell/addsomain";
      this.$store.commit("resetSo", {});

      this.ruleForm.soId = new Date().getTime();
      this.getTime();
      this.ruleForm.account = this.loginUser;

      this.$store.commit('resetSo',this.ruleForm)
    },
    // 分页
    changePage(p) {
      this.page = p;
    ...
    getEditQueryItem() {
      axios({
        ...
        method: "get",
        params: { soId: this.ruleForm.soId }
      }).then(resp => {
        console.log(resp);
        this.ruleForm.soitems = resp.data;
        this.$store.commit("addSoDetail", this.ruleForm);
        location.href = "http://localhost:8080/#/sell/addsomain";
      });
    },
    // 修改新增状态的采购单
     handleEdit(index, row) {
    //   this.$store.commit("addSoDetail", index);
        this.editPo = this.somainList[index];
